        ...
        try {
            ...
            if (oldGridfsId && oldGridfsId.toString() !== gridfsId.toString()) {
                try {
                    ...
                    console.log(`Köhnə fayl silindi: sinif ${sinif}, gridfs_id: ${oldGridfsId}`);
                } catch (deleteErr) {
                    // Əgər fayl tapılmadısa, xəta vermə, sadəcə log yaz
                    console.log(`Köhnə fayl tapılmadı və ya silinə bilmədi: ${deleteErr.message}`);
                }
            }
            ...
            if (!res.headersSent) {
                res.status(500).json({ error: 'Fayl yüklənərkən xəta baş verdi: ' + uploadErr.message });
            }
        }
...
app.get('/api/download-pdf/:sinif', async (req, res) => {
    ...
    try {
        ...
        if (pdfRecord.gridfs_id) {
            ...
            res.setHeader('Content-Disposition', `attachment; filename="${pdfRecord.fayl_adi}"`);
...
app.get('/api/view-pdf/:sinif', async (req, res) => {
    ...
    try {
        ...
        if (pdfRecord.gridfs_id) {
            ...
            const fileInfo = await filesCollection.findOne({ _id: pdfRecord.gridfs_id });
    ...
    try {
        ...
        for (const row of allRows) {
            ...
            if (row.gridfs_id) {
                try {
                    const filesCollection = db.collection('sinif_pdfs.files');
                    const fileInfo = await filesCollection.findOne({ _id: row.gridfs_id });
                    if (fileInfo) {
                        fileExists = true;
                    }
                } catch (err) {
                    console.log(`GridFS fayl yoxlanılarkən xəta (sinif ${row.sinif}):`, err.message);
                }
            }
            ...
            if (fileExists) {
                ...
                console.log(`Mövcud olmayan fayl metadata-sı silinir: sinif ${row.sinif}`);
                try {
                    await pdfCollection.deleteOne({ _id: row._id });
                } catch (deleteErr) {
                    console.error(`Metadata silinərkən xəta (sinif ${row.sinif}):`, deleteErr.message);
                }
            }
        }
